...
bool Dictionary::is_word(const string& word) const {
    ...
    return cur->is_final;  // checks whether the node at the end of the prefix makes a valid word
}
...
vector<char> Dictionary::next_letters(const std::string& prefix) const {
    ...
    for (it = cur->nexts.begin(); it != cur->nexts.end(); it++) {  // iterates over cur->nexts
