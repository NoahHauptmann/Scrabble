...
void Scrabble::game_loop() {
    ...
    while (1) {
        ...
        while (1) {
            try {
                board.print(cout);  // prints out the board at the beginning of each turn
        ...
        if (player_move.kind == MoveKind::PASS) {
            ...
            if (consecutive_pass_count >= num_human_players) {  // if everyone has passed, the game is over
...
void Scrabble::add_players() {
    ...
    for (int i = 0; i < num_players; i++) {
        cout << "Please input the name for player " << i << ": ";
        string player_name;
        getline(cin, player_name);  // reads in a line for each player's name
        cout << "Is " << player_name << " a computer? (y/n)"
             << endl;  // asks whether or not the new player is a computer
        string player_type;
        getline(cin, player_type);
        shared_ptr<Player> new_player;
        if (player_type == "y") {
            new_player = make_shared<ComputerPlayer>(player_name, hand_size);      // makes a new computer player object
            new_player->add_tiles(this->tile_bag.remove_random_tiles(hand_size));  // gives the new player tiles
        } else {
            num_human_players++;
            new_player = make_shared<HumanPlayer>(player_name, hand_size);         // makes a new human player object
            new_player->add_tiles(this->tile_bag.remove_random_tiles(hand_size));  // gives the new player tiles
        }
