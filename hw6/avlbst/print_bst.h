...
int getNodeDepth(
        BinarySearchTree<Key, Value> const& tree,
        std::shared_ptr<Node<Key, Value>> root,
        std::shared_ptr<Node<Key, Value>> node) {
    int dist = 1;
...
#define ELEMENT_WIDTH (BOX_WIDTH + PADDING)
    ...
    std::vector<std::shared_ptr<Node<Key, Value>>>
    ...
    for (size_t levelIndex = 0; levelIndex < printedTreeHeight; ++levelIndex) {
        ...
        currRowNodes.clear();
        for (typename std::vector<std::shared_ptr<Node<Key, Value>>>::iterator prevRowIter = prevRowNodes.begin();
