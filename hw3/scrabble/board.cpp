...
PlaceResult Board::test_place(const Move& move) const {
    ...
    if (move.kind != MoveKind::PLACE) {
        ...
        if (move.direction == Direction::ACROSS) {  // move across
            ...
            for (unsigned int i = move.column; i < (move.column + move.tiles.size());
                ...
                if (move.tiles[i - (move.column)].letter
                    == move.tiles[i - (move.column)].BLANK_LETTER) {  // checks for blank tiles
                    verticalword.push_back(move.tiles[i - (move.column)].assigned);
                } else {
                    verticalword.push_back(move.tiles[i - (move.column)].letter);
                }
            ...
            for (unsigned int i = move.row; i < (move.row + move.tiles.size());
                ...
                if (move.tiles[i - (move.row)].letter
                    == move.tiles[i - (move.row)].BLANK_LETTER) {  // checks for blank tile
                    horizontalword.push_back(move.tiles[i - (move.row)].assigned);
                } else {
                    horizontalword.push_back(move.tiles[i - (move.row)].letter);
                }
                horizontal_word_points
                        += move.tiles[i - (move.row)].points
                           * this->at(currentposition_left).letter_multiplier;  // adds points of placed tiles
                currentposition_left.column++;
